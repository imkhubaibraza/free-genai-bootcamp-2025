...
class StudyController {
    static async getStudySessions(req, res, next) {
        try {
    ...
    static async getStudySession(req, res, next) {
        try {
            const db = await (0, database_1.getDatabase)();
      ...
      `, req.params.id);
            if (!session) {
                throw new errorHandler_1.AppError(404, 'Study session not found');
            }
            res.json(session);
        }
        catch (error) {
    ...
    static async getStudySessionWords(req, res, next) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 100;
            const offset = (page - 1) * limit;
    ...
    static async createStudySession(req, res, next) {
      ...
      `, [group_id, study_activity_id]);
            if (!result.lastID) {
                throw new errorHandler_1.AppError(500, 'Failed to create study session');
            }
            res.status(201).json({
                id: result.lastID,
                group_id
    ...
    static async reviewWord(req, res, next) {
        try {
      ...
      `, [word_id, study_session_id, correct]);
            if (!result.lastID) {
                throw new errorHandler_1.AppError(500, 'Failed to record word review');
            }
